...
# 教练更新预约状态
@app.route("/api/coach/appointment/<int:appointment_id>/status", methods=["POST"])
def coach_update_appointment_status(appointment_id):
    if "role" not in session or session["role"] != "coach":
        return jsonify({"error": "未授权"}), 403
    
    data = request.get_json()
    new_status = data.get("status")
    valid_status = ["confirmed", "rejected", "cancelled"]
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            ...
            """, (new_status, appointment_id, coach_id))

            # 查找预约的学生信息
            cursor.execute("SELECT student_id, start_time FROM appointments WHERE id=%s", (appointment_id,))
            appt = cursor.fetchone()

            if appt:
                student_id = appt["student_id"]
                start_time = appt["start_time"].strftime("%Y-%m-%d %H:%M") if appt["start_time"] else ""

                if new_status == "confirmed":
                    content = f"您的课程预约（ID {appointment_id}，时间 {start_time}）已被教练确认。"
                    msg_type = "notification"
                elif new_status == "rejected":
                    content = f"您的课程预约（ID {appointment_id}，时间 {start_time}）已被教练拒绝。"
                    msg_type = "approval_request"
                elif new_status == "cancelled":
                    content = f"您的课程（ID {appointment_id}，时间 {start_time}）已被教练取消。"
                    msg_type = "cancel_request"
                else:
                    content, msg_type = None, None

                if content:
                    cursor.execute("""
                        INSERT INTO messages (sender_id, receiver_id, content, type, related_id, is_read)
                        VALUES (%s, %s, %s, %s, %s, 0)
                    """, (coach_id, student_id, content, msg_type, appointment_id))

        conn.commit()
        log_action("Coach updated appointment", f"Appointment ID {appointment_id}, New Status {new_status}")
        return jsonify({"message": f"预约已更新为 {new_status}"})
    finally:
        conn.close()
...
# 获取教练可预约时间
@app.route("/api/coach/availability", methods=["GET"])
def get_coach_availability():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                ...
                WHERE coach_id=%s AND date BETWEEN %s AND %s
            """, (coach_id, today, next_week))
            rows = cursor.fetchall()
    finally:
        conn.close()
    ...
    for i in range(7):
        ...
        slots = [r["slot"] for r in rows if r["date"] == d]
        result.append({"date": str(d), "slots": slots})
    return jsonify(result)


# 保存教练可预约时间
@app.route("/api/coach/availability", methods=["POST"])
def save_coach_availability():
    ...
    # slots 示例: ["2025-09-25 09:00-10:00", "2025-09-25 10:00-11:00", ...]

    conn = get_db_connection()
    try:
        with conn.cursor() as cursor:
            # 先清空未来一周的设置
            cursor.execute("DELETE FROM coach_availability WHERE coach_id=%s AND date >= CURDATE()", (coach_id,))
            # 重新插入
            for s in slots:
                date_str, slot = s.split(" ")
                cursor.execute("""
                    INSERT INTO coach_availability (coach_id, date, slot)
                    VALUES (%s, %s, %s)
                """, (coach_id, date_str, slot))
        conn.commit()
        log_action("Updated availability", f"Coach ID: {coach_id}, Slots: {len(slots)}")
        return jsonify({"message": "可预约时间已保存"})
    finally:
        conn.close()

# 获取教练相关的评价
@app.route("/api/coach/evaluations", methods=["GET"])
def api_coach_evaluations():
    if "role" not in session or session["role"] != "coach":
        return jsonify({"error": "未授权"}), 403
        ...
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            sql = """
                SELECT a.id AS appointment_id, a.start_time,
                       ...
                       e.student_feedback, e.coach_feedback
                FROM appointments a
                JOIN users u ON a.student_id = u.id
                LEFT JOIN evaluations e ON e.appointment_id = a.id
                WHERE a.coach_id = %s AND a.status='completed'
                ORDER BY a.start_time DESC
            """
            cursor.execute(sql, (coach_id,))
            rows = cursor.fetchall()
...
# 教练提交评价
@app.route("/api/coach/evaluations/submit", methods=["POST"])
def api_coach_submit_evaluation():
    if "role" not in session or session["role"] != "coach":
        return jsonify({"error": "未授权"}), 403
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            # 确认这节课属于当前教练
            cursor.execute("SELECT coach_id FROM appointments WHERE id=%s", (appointment_id,))
            appt = cursor.fetchone()
            if not appt or appt["coach_id"] != session.get("user_id"):
                return jsonify({"message": "无权限操作"}), 403

            # 检查 evaluations 记录
            cursor.execute("SELECT id, coach_feedback FROM evaluations WHERE appointment_id=%s", (appointment_id,))
            row = cursor.fetchone()
            if row:
                if row["coach_feedback"]:
                    return jsonify({"message": "该课程已提交过评价"}), 400
                else:
                    cursor.execute(
                        "UPDATE evaluations SET coach_feedback=%s WHERE appointment_id=%s",
                        (comment, appointment_id)
                    )
            else:
                cursor.execute(
                    "INSERT INTO evaluations (appointment_id, coach_feedback) VALUES (%s, %s)",
                    (appointment_id, comment)
                )
        conn.commit()
        log_action("Coach submitted evaluation", f"Appointment {appointment_id}, Feedback: {comment}")
        return jsonify({"message": "评价提交成功"})
    finally:
        conn.close()
...
# 获取教练消息
@app.route("/api/coach/messages", methods=["GET"])
def api_coach_messages():
    if "role" not in session or session["role"] != "coach":
        return jsonify({"error": "未授权"}), 403
...
# 标记消息为已读
@app.route("/api/coach/messages/<int:msg_id>/read", methods=["POST"])
def api_coach_message_read(msg_id):
    if "role" not in session or session["role"] != "coach":
        return jsonify({"error": "未授权"}), 403
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """, (msg_id, coach_id))
        conn.commit()
        log_action("Message marked as read", f"Coach ID {coach_id}, Msg {msg_id}")
        return jsonify({"message": "消息已标记为已读"})
    finally:
        conn.close()
